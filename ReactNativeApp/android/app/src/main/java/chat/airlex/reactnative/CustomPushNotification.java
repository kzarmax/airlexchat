...
public class CustomPushNotification extends PushNotification {
    ...
    public void onReceived() throws InvalidNotificationException {
        ...
        String badgeCount = bundle.getString("msgcnt", "0");

        Log.d("AirlexChat Notification", mNotificationProps.toString());
        ...
        // Active Screen When Sleep mode
        PowerManager pm = (PowerManager) reactApplicationContext.getSystemService(Context.POWER_SERVICE);
        @SuppressLint("InvalidWakeLockTag") PowerManager.WakeLock wl =  pm.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP
                | PowerManager.ON_AFTER_RELEASE, "My Tag");
        wl.acquire(3000);
        ///  ..screen will stay on during this section..
        wl.release();

        // Set Badge (For Special Android Type as "Xperia" ... )
        if(this.isBadgeCounterSupported(mContext)) {
            ShortcutBadger.applyCount(mContext, Integer.parseInt(badgeCount));
        }

        // Ring
        if(!title.isEmpty() && loadedEjson.messageType != null && (loadedEjson.messageType.equals("jitsi_call_started") || loadedEjson.messageType.equals("jitsi_video_call_started"))){
            try {
                Uri notification = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE);
                MediaPlayer mp = MediaPlayer.create(reactApplicationContext, notification);
                mp.setLooping(false);
                mp.start();
                new CountDownTimer(5000, 1000) {
                    public void onTick(long millisUntilFinished) {
                        if(millisUntilFinished > 5000) mp.stop();
                    }
    ...
    private boolean isBadgeCounterSupported(Context context) {
        if (Build.MANUFACTURER.equalsIgnoreCase("Samsung") && Build.VERSION.SDK_INT >= 26) {
            Log.d("AirlexChat", "Launcher icon badge (ShortcutBadger) is not supported on Samsung devices running Android 8 (or newer).");
            return false;
        }
    ...
    protected Notification.Builder getNotificationBuilder(PendingIntent intent) {
        ...
        if(title.isEmpty() && message.isEmpty()){       // Notification Disabled - Only Badge
            privateNotificationChannel(notification);
        } else {
    ...
    private void notificationChannel(Notification.Builder notification) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String CHANNEL_ID = "airlexchat_channel_01";
    ...
    private void privateNotificationChannel(Notification.Builder notification) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String CHANNEL_ID = "airlexchat_channel_private";
