...
import {removedRoom, setRoom} from '../actions/room';
import RocketChat from '../lib/rocketchat';
import log, { logEvent, events } from '../utils/log';
import I18n from '../i18n';
import { showErrorAlert } from '../utils/info';

const watchUserTyping = function* watchUserTyping({ rid, username, status }) {
	...
	try {
		yield RocketChat.emitTyping(rid, username, status);

		if (status) {
			yield delay(5000);
			yield RocketChat.emitTyping(rid, username, false);
		}
	} catch (e) {
		log(e);
	}
...
const handleLeaveRoom = function* handleLeaveRoom({ rid, t, cardId }) {
	logEvent(events.RA_LEAVE);
	try {
		const result = yield RocketChat.leaveRoom(rid, t, cardId);
		if (result.success) {
...
const handleDeleteRoom = function* handleDeleteRoom({ rid, t, cardId }) {
	logEvent(events.RI_EDIT_DELETE);
	try {
		const result = yield RocketChat.deleteRoom(rid, t, cardId);
		if (result.success) {
			yield handleRemovedRoom();
		}
	} catch (e) {
