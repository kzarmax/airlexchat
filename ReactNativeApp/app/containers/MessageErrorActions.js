...
const MessageErrorActions = forwardRef(({ tmid }, ref) => {
	...
	const handleResend = protectedFunction(async(message, cardId) => {
		await RocketChat.resendMessage(message, cardId, tmid);
	});
	...
	const showMessageErrorActions = (message, cardId) => {
		showActionSheet({
			options: [
				{
					title: I18n.t('Resend'),
					icon: 'send',
					onPress: () => handleResend(message, cardId)
				},
				{
					title: I18n.t('Delete'),
					icon: 'delete',
