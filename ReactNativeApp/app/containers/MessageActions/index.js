...
const MessageActions = React.memo(forwardRef(({
	...
	const isOwn = message => message.cardId === card._id;

	const allowEdit = (message) => {
		if (isReadOnly || !message.msg || !message.msg.length) {
			return false;
		}
	...
	const handleReaction = (shortname, message) => {
		...
		if (shortname) {
			onReactionPress(shortname, message.id, card._id);
		} else {
		...
		showConfirmationAlert({
			...
			onPress: async() => {
				try {
					...
					await RocketChat.deleteMessage(message.id, message.subscription.id, card._id);
				} catch (e) {
	...
	const getOptions = (message) => {
		...
		// Reply
		if (!isReadOnly && !message.tmid && !tmid && message.msg && message.msg.length) {
			options = [{
				title: I18n.t('Reply'),
				icon: 'threads',
