...
async function load({ tmid, cardId, offset }) {
	try {
		// RC 1.0
		const result = await this.sdk.get('chat.getThreadMessages', {
			tmid, cardId, count: 50, offset, sort: { ts: -1 }, query: { _hidden: { $ne: true } }
		});
...
export default function loadThreadMessages({ tmid, rid, cardId, offset = 0 }) {
	return new Promise(async(resolve, reject) => {
		try {
			let data = await load.call(this, { tmid, cardId, offset });
