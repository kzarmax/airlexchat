...
const createOrUpdateSubscription = async(subscription, room) => {
	try {
		...
		log(e, 'createOrUpdateSubscription Error: ');
	}
};
...
export default function subscribeRooms() {
	...
	const handleRemovedRoom = async (rid) => {
		try {
			const db = database.active;
			const subCollection = db.collections.get('subscriptions');
			...
			if (rid === roomState.rid && roomState.isDeleting) {
				store.dispatch(removedRoom());
			} else {
				EventEmitter.emit('ROOM_REMOVED', { rid });
			}
		} catch (e) {
			log(e);
		}
	...
	const handleStreamMessageReceived = protectedFunction(async(ddpMessage) => {
		...
		if (/userData/.test(ev)) {
			...
			const server = await UserPreferences.getStringAsync(RocketChat.CURRENT_SERVER);
			// Update User Points
			if(diff?.points){
				try {
					const userId = await UserPreferences.getStringAsync(`${ RocketChat.TOKEN_KEY }-${ server }`);
					const serverDb = database.servers;
					const userCollection = serverDb.collections.get('users');
					const userRecord = await userCollection.find(userId);
					await serverDb.action(async() => {
						await userRecord.update((u) => {
							u.points = diff.points;
						});
					});
				} catch {
				}
				store.dispatch(setUser({points: diff.points}));
			}
			...
			if(diff?.emojis){
				try {
					...
					await serverDb.action(async() => {
						await userRecord.update((u) => {
							u.emojis = diff.emojis;
						});
					});
				} catch {
				}
				store.dispatch(setUser({emojis: diff.emojis}));
			}
		}
		LOG_L_MIDDLE(`subscribeRooms: handleStreamMessageReceived => ${ev}  type: ${type} `, data);
		if (/subscriptions/.test(ev)) {
			if (type === 'removed') {
				await handleRemovedRoom(data.rid);
			} else {
		...
		if (/message/.test(ev)) {
			...
			if(args.msg === '無効なルーム'){
				return await handleRemovedRoom(args.rid);
			}
			const _id = random(17);
			const message = {
				_id,
