...
async function load({ rid: roomId, cardId, lastOpen }) {
	let lastUpdate;
	if (lastOpen) {
	...
	// RC 0.60.0
	const { result } = await this.sdk.get('chat.syncMessages', { roomId, cardId, lastUpdate });
	return result;
}
...
export default function loadMissedMessages(args) {
	return new Promise(async(resolve, reject) => {
		try {
			const data = (await load.call(this, { rid: args.rid, cardId:args.cardId, lastOpen: args.lastOpen }));
