...
export default class EncryptionRoom {
	...
	importRoomKey = async(E2EKey, privateKey) => {
		...
		try {
			decryptedKey = await SimpleCrypto.RSA.decrypt(roomE2EKey, privateKey);
		} catch (e) {
			log(e, 'decryption failed');
		}

		if (!decryptedKey) {
			decryptedKey = await RocketChat.e2eDecryptByNodeJSCrypto(roomE2EKey, privateKey);
		}
	...
	createRoomKey = async() => {
		...
		// Fix bug already set key
		const { success } = await RocketChat.e2eSetRoomKeyID(this.roomId, this.keyID);
		if (!success) {
			return false;
		}
