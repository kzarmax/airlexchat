...
import {E2E_MESSAGE_TYPE, E2E_PRIVATE_KEY, E2E_PUBLIC_KEY, E2E_RANDOM_PASSWORD_KEY, E2E_STATUS} from './constants';
...
class Encryption {
	...
	createKeys = async(userId, server) => {
		...
		const encodedPrivateKey = await this.encodePrivateKey(EJSON.stringify(privateKey), AIRLEX_DEFAULT_PASSWORD, userId);
			...
			// If the subscription already exists but doesn't have the E2EKey yet
				...
				if (!subRecord._hasPendingUpdate) {
					...
					batch.push(subRecord.prepareUpdate((s) => {
						...
						s.e2eKeyId = subscription.e2eKeyId;
					}));
				}
			}
	...
	encryptMessage = async(message) => {
		...
		// Not Encrypt message with Url or Mention (Server not parsing)
		if(/http(s)?:\/\//i.test(msg) || /@/i.test(msg)){
			return message;
		}
		const db = database.active;
		const subCollection = db.collections.get('subscriptions');
