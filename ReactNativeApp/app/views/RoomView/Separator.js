...
const DateSeparator = React.memo(({ ts, unread, textColor, theme }) => {
	...
	if (ts) {
		return (
			<View style={styles.container}>
				<View style={[styles.line, { backgroundColor: textColor?textColor:themes[theme].borderColor }]} />
				<Text style={[styles.text, { color: textColor?textColor:themes[theme].auxiliaryText }, styles.marginLeft]}>{date}</Text>
			</View>
		);
	}
	return (
