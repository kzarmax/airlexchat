...
import { getSystemVersion, isIOS } from '../utils/deviceInfo';
...
const userAgent = isIOS
	...
	: `Mozilla/5.0 (Linux; Android ${getSystemVersion};) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.98 Mobile Safari/537.36`;
...
class AuthenticationWebView extends React.PureComponent {
	static navigationOptions = ({ navigation }) => ({
		...
		headerLeft: () => <HeaderButton.CloseModal navigation={navigation} testID='oauth-view-close' />
	})
	...
	login = async(params) => {
		...
		try {
			const isVerify = this.props.route.params?.isVerify;
			if(isVerify){
				const result = RocketChat.login(params);
				if(result){
					const cardId = this.props.route.params?.cardId;
					navigation.replace("ResetCardPasswordView", { cardId });
					return;
				} else {
					showToast(I18n.t("error-not-authorized"));
				}
			} else {
