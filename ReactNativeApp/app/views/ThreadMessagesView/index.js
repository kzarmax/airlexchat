...
class ThreadMessagesView extends React.Component {
	...
	getHeader = () => {
		...
		options.headerRight = () => (
			<HeaderButton.Container>
				...
				<HeaderButton.Item title='menu' iconName='filter' onPress={this.showFilterDropdown} testID='rooms-list-view-create-channel' />
			</HeaderButton.Container>
		);
		return options;
	}
		...
		try {
			...
			this.messagesSubscription = this.messagesObservable
				.subscribe((messages) => {
					...
					if (this.mounted) {
						this.setState({ messages, displayingThreads, initLoading: false });
					} else {
						this.state.messages = messages;
						this.state.displayingThreads = displayingThreads;
						this.state.initLoading = false;
					}
				});
		} catch (e) {
			log(e);
	...
	load = debounce(async(lastThreadSync) => {
		...
		try {
			const result = await RocketChat.getThreadsList({
				rid: this.rid, cardId: this.cardId, count: API_FETCH_COUNT, offset: messages.length, text: searchText
			});
			if (result.success) {
	...
	sync = async(updatedSince) => {
		...
		try {
			const result = await RocketChat.getSyncThreadsList({
				rid: this.rid, cardId: this.cardId, updatedSince: updatedSince.toISOString()
			});
	...
	getFilteredThreads = (messages, subscription, currentFilter) => {
		...
		if (currentFilter === FILTER.FOLLOWING) {
			return messages?.filter(item => item?.replies?.find(c => c === this.cardId));
	...
	toggleFollowThread = async(isFollowingThread, tmid) => {
		try {
			await RocketChat.toggleFollowMessage(tmid, this.cardId, !isFollowingThread);
	...
	renderContent = () => {
		const {
			initLoading, loading, messages, displayingThreads, currentFilter
		} = this.state;
		const { theme } = this.props;
