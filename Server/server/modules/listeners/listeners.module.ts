...
export class ListenersModule {
	constructor(
		...
		service.onEvent('card.avatarUpdate', ({ _id, avatarETag: etag }) => {
			notifications.notifyLoggedInThisInstance('updateAvatar', {
				_id,
				etag,
			});
		...
		service.onEvent('card.imageUpdate', ({ _id, avatarETag: etag }) => {
			notifications.notifyLoggedInThisInstance('updateAvatar', {
				_id,
				etag,
			});
		...
		service.onEvent('card.nameChanged', (card) => {
			notifications.notifyLoggedInThisInstance('Users:NameChanged', card);
		});
	}
}
