...
import {IPublication, IStreamer, IStreamerConstructor} from '../streamer/streamer.module';
...
export class NotificationsModule {
	...
	async configure({ Rooms, Subscriptions, Users, Settings }: IModelsParam): Promise<void> {
		...
		this.streamRoomMessage.allowEmit('__my_messages__', async function(_eventName, { rid, cardId }) {
			if (!this.userId) {
				return false;
			}
			...
			try {
				...
				const canAccess = await Authorization.canAccessRoom(room, { _id: cardId });
				if (!canAccess) {
					return false;
				}

				const roomParticipant = await Subscriptions.findOneByRoomIdAndCardId(room._id, cardId);
