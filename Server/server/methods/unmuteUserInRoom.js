...
Meteor.methods({
	unmuteUserInRoom(data) {
		...
		const card = Cards.findOneById(data.username);
		const unmutedUser = Users.findOneById(card.userId);

		const fromUser = Users.findOneById(fromId);

		callbacks.run('beforeUnmuteUser', { unmutedUser, fromUser }, room);

		Rooms.unmuteCardIdByRoomId(data.rid, card._id);

		Messages.createUserUnmutedWithRoomIdAndCard(data.rid, card, {
			u: {
				_id: fromUser._id,
