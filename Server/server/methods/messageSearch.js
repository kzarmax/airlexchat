...
import { Subscriptions, Cards } from '../../app/models/server';
...
Meteor.methods({
	messageSearch(text, rid, cardId, limit) {
		check(text, String);
		...
		const card = Cards.findOneByIdAndUserId(cardId, currentUserId);
		if (!card) {
			throw new Meteor.Error('error-invalid-user', 'Invalid user', {
				method: 'messageSearch',
			});
		...
		// Query for senders
		const from = [];
		text = text.replace(`/from:(${ settings.get('UTF8_Names_Validation') })/ig`, function(match, username) {
		...
		// Query for senders
		const mention = [];
		text = text.replace(`/mention:(${ settings.get('UTF8_Names_Validation') })/ig`, function(match, username) {
		...
		if (Object.keys(query).length > 0) {
			...
			if (rid) {
				...
				query.rid = {
					$in: Subscriptions.findByCardId(card._id)
						.fetch()
