...
import { Users, Settings, Cards } from '../../app/models';
...
Meteor.startup(function() {
	Meteor.defer(() => {
		...
		if (!Users.findOneById('airlex.chat')) {
			Users.create({
				_id: 'airlex.chat',
				name: 'エアレペルソナ',
				username: 'airlex.chat',
				status: 'online',
				statusDefault: 'online',
				utcOffset: 0,
				active: true,
			...
			addUserRoles('airlex.chat', 'bot');

			const buffer = Buffer.from(Assets.getBinary('avatars/airlexchat.png'));

			const rs = RocketChatFile.bufferToStream(buffer, 'utf8');
			const fileStore = FileUpload.getStore('Avatars');
			fileStore.deleteByName('airlex.chat');
			...
			Meteor.runAsUser('airlex.chat', () => {
				fileStore.insert(file, rs, () => Cards.setAvatarOrigin('airlex.chat', 'local', null));
			});
		}
