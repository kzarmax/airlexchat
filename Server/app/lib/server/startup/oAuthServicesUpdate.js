	...
	services.forEach((service) => {
		...
		if (service.value === true) {
			...
			if (serviceName === 'Apple'){
				data.teamId = settings.get(`Accounts_OAuth_Apple_team_id`);
				data.redirectUri = `${ settings.get('Site_Url') }/${settings.get(`Accounts_OAuth_Apple_callback_url`)}?close`;
				data.keyId = settings.get(`Accounts_OAuth_Apple_key_id`);
			}
