...
import { Subscriptions, Users, Cards } from '../../../models/server';
...
export const sendNotification = async ({
	...
	if (subscription.c._id === sender._id) {
		return;
	}

	// Not Send Notification to Secret Card
	const targetCard = Cards.findOneById(subscription.c._id);
	if(!targetCard || targetCard.isSecret){
		return;
	}

	const hasMentionToUser = mentionIds.includes(subscription.c._id);
...
export async function sendMessageNotifications(message, room, cardsInThread = []) {
	if (TroubleshootDisableNotifications === true) {
		return;
	}

	const sender = roomTypes.getConfig(room.t).getMsgSender(message.c._id);
	if (!sender) {
		return message;
	}

	const mentionIds = (message.mentions || []).map(({ _id }) => _id).concat(cardsInThread); // add users in thread to mentions array because they follow the same rules
	...
	const query = {
		...
		$or: [
			...
			...cardsInThread.length > 0 ? [{ 'c._id': { $in: cardsInThread } }] : [],
		],
	};
	...
	['audio', 'desktop', 'mobile', 'email'].forEach((kind) => {
		...
		if (mentionIdsWithoutGroups.length > 0) {
			query.$or.push({
				...
				'c._id': { $in: mentionIdsWithoutGroups },
			});
	...
	subscriptions.forEach((subscription) => sendNotification({
		...
		mentionIds,
		disableAllMessageNotifications,
		hasReplyToThread: cardsInThread && cardsInThread.includes(subscription.c._id),
	}));
...
export async function sendAllNotifications(message, room) {
	...
	if (room.t === 'c') {
		...
		Subscriptions.findByRoomIdAndCardIds(room._id, mentionIdsWithoutGroups, { fields: { 'c._id': 1 } }).forEach((subscription) => {
			const index = mentions.indexOf(subscription.c._id);
			if (index !== -1) {
