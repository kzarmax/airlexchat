...
import { Rooms, Cards, Subscriptions } from '../../../models';

export const getRoomByNameOrIdWithOptionToJoin = function _getRoomByNameOrIdWithOptionToJoin({ currentUserId, cardId, nameOrId, type = '', tryDirectByCardIdOnly = false, joinChannel = true, errorOnEmpty = true }) {
	let room;
	...
	if (nameOrId.startsWith('#')) {
		...
		let roomUser;
		if (tryDirectByCardIdOnly) {
			roomUser = Cards.findOneById(nameOrId);
		} else {
			// FIXME: usernameあてないほうがいいかも
			roomUser = Cards.findOne({
				$or: [{ _id: nameOrId }, { username: nameOrId }],
			});
		}

		const rid = _.isObject(roomUser) ? [cardId, roomUser._id].sort().join('') : nameOrId;
		...
		if (!_.isObject(room)) {
			...
			room = Meteor.runAsUser(currentUserId, function() {
				const { rid } = Meteor.call('createDirectMessage', cardId, roomUser._id);
	...
	if (room.t === 'c' && joinChannel) {
		const sub = Subscriptions.findOneByRoomIdAndCardId(room._id, cardId);
