...
import { Users, Messages, Cards } from '../../../models';
...
export function executeSendMessage(uid, message) {
	...
	try {
		...
		if(user.deviceInfo){
			const versions = user.deviceInfo.split('.');
			if(versions && versions.length > 3 &&
				(versions[0] < '1' ||
					(versions[0] === '1' && versions[1] < '3') ||
					(versions[0] === '1' && versions[1] === '3' && versions[2] < '6'))
			){
				throw new Error('アプリを最新バージョンにアップグレードしてください。');
			}
		}

		const room = canSendMessage(rid, { uid, cardId:message.cardId, username: user.username, type: user.type });
		...
		api.broadcast('notify.ephemeralMessage', uid, message.rid, {
			msg: TAPi18n.__(errorMessage, {}, 'ja'),
		});
