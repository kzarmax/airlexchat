...
const getServerForUser = (user) => {
	if(user.server) return user.server;
	if(!settings.get('Allow_Distribute_Server')){
		const server = settings.get('Site_Url');
		return server.endsWith('/')?server.slice(0, -1):server;
	}
	const serverUrls = settings.get('Server_Urls');
	const servers = serverUrls.split(',')
		.filter((s) => {
	...
	let serverUsers = servers.map((server)=>{
		return {
			server: server,
			users: Meteor.users.find({ server: server }).count()
		};
	});
	serverUsers = serverUsers.sort((a, b) => a.users - b.users);
	const selectServer = serverUsers[0].server;
	...
	_initAuth() {
		...
		this.addRoute('login', { authRequired: false }, {
			post() {
				...
				if(!this.user.server){
					const userServer = getServerForUser(this.user);
					this.user = {
						...this.user,
						server: userServer
					};
