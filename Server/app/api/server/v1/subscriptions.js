...
API.v1.addRoute('subscriptions.getOne', { authRequired: true }, {
	get() {
		const { roomId, cardId } = this.requestParams();
...
API.v1.addRoute('subscriptions.read', { authRequired: true }, {
	post() {
		...
		Meteor.runAsUser(this.userId, () =>
			Meteor.call('readMessages', this.bodyParams.rid, this.bodyParams.cardId),
		);
...
API.v1.addRoute('subscriptions.unread', { authRequired: true }, {
	post() {
		const { roomId, cardId, firstUnreadMessage } = this.bodyParams;
		if (!cardId) {
			return API.v1.failure('The required "cardId" param is missing.');
		}
		...
		Meteor.runAsUser(this.userId, () =>
			Meteor.call('unreadMessages', firstUnreadMessage, roomId, cardId)
		);
