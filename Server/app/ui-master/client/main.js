...
Template.main.helpers({
	removeSidenav() {
		...
		return (Layout.isEmbedded() && !/^\/admin/.test(FlowRouter.current().route.path)) || !(userId && hasRole(userId, 'admin'));
	},
	isAdmin(){
	...
	hasUsername() {
		...
		const user = uid && Users.findOne({ _id: uid }, { fields: { username: 1, roles: 1 } });
		return (user && !user.roles.includes('admin')) || (user && user.username) || (!uid && settings.get('Accounts_AllowAnonymousRead'));
	},
