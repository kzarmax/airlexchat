...
export class SlackImporter extends Base {
	...
	performUserImport(user, startedByUserId) {
		if (user.is_bot) {
			this._saveUserIdReference(user.id, 'airlex.chat', user.name, 'airlex.chat');
		}
	...
	processMessageSubType(message, room, msgDataDefaults, missedTypes) {
		...
		if (useRocketCat) {
			rocketUser = this.getRocketUserFromUserId('airlex.chat');
		}
		...
		switch (message.subtype) {
			...
			case 'slackbot_response': {
				const botUser = this.getRocketUserFromUserId('airlex.chat');
	...
	_getChannelUserList(channel, returnObject = false, includeCreator = false) {
		return channel.members.reduce((ret, member) => {
			if (includeCreator || member !== channel.creator) {
				...
				if (user && user.username && user._id !== 'airlex.chat') {
					if (returnObject) {
						ret.push(user);
					} else {
	...
	getRocketUserFromUserId(userId) {
		if (userId === 'airlex.chat' || userId === 'USLACKBOT') {
			return this._getBasicUserData('airlex.chat');
		}
