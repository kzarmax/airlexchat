...
import CardImages from '../../../models/server/models/CardImages';
import GroupImages from '../../../models/server/models/GroupImages';
import Users from '../../../models/server/models/Users';
import Rooms from '../../../models/server/models/Rooms';
import Cards from '../../../models/server/models/Cards';
...
export const FileUpload = {
	...
	validateFileUpload(file) {
		...
		const card = file.cardId ? Cards.findOneById(file.cardId) : null;
		...
		if (user?.type !== 'app' && canAccessRoom(room,  card, file) !== true) {
			return false;
		}
		const language = user ? user.language : 'jp';
	...
	validateAvatarUpload(file) {
		...
		const language = user?.language || 'jp';
	...
	defaultUploads() {
		return {
			...
			getPath(file) {
				return `${ settings.get('uniqueID') }/uploads/${ file.rid }/${ file.cardId }/${ file._id }`;
			},
	...
	defaultAvatars() {
		return {
			...
			getPath(file) {
				const avatarFile = file.rid ? `room-${ file.rid }` : file.cardId;
	...
	defaultCardImages() {
		// console.log('FileUpload.defaultCardImages');
		return {
			...
			// }),
			getPath(file) {
				// console.log('defaultCardImages.getPath :', file);
				return `${ settings.get('uniqueID') }/cardImages/${ file.cardId }`;
			},
			onValidate: FileUpload.cardImagesOnValidate,
			onFinishUpload: FileUpload.cardImagesOnFinishUpload,
		};
	},

	defaultGroupImages() {
		// console.log('FileUpload.defaultGroupImages');
		return {
			...
			// }),
			getPath(file) {
				// console.log('defaultGroupImages.getPath :', file);
				return `${ settings.get('uniqueID') }/groupImages/${ file.rid }`;
			},
			onValidate: FileUpload.groupImagesOnValidate,
			onFinishUpload: FileUpload.groupImagesOnFinishUpload,
		};
	},
	...
	cardImagesOnValidate(file) {
		// console.log('FileUpload.cardImagesOnValidate :', file);
		if (settings.get('Accounts_AvatarResize') !== true) {
			return;
		}
	...
	groupImagesOnValidate(file) {
		// console.log('FileUpload.groupImagesOnValidate :', file);
		if (settings.get('Accounts_AvatarResize') !== true) {
			return;
		}
		if (Meteor.userId() !== file.userId && !hasPermission(Meteor.userId(), 'edit-other-user-info')) {
	...
	avatarsOnFinishUpload(file) {
		...
		if(file.cardId){
			const card = Cards.findOneById(file.cardId);
			const oldAvatar = Avatars.findOneByName(card._id);
			if (oldAvatar) {
				Avatars.deleteFile(oldAvatar._id);
			}
			Avatars.updateFileNameById(file._id, card._id);
		}
		// console.log('upload finished ->', file);
	},

	cardImagesOnFinishUpload(file) {
		if (Meteor.userId() !== file.userId && !hasPermission(Meteor.userId(), 'edit-other-user-info')) {
			throw new Meteor.Error('error-not-allowed', 'Change card image is not allowed');
		}
		// update file record to match card id
		const card = Cards.findOneById(file.cardId);
		const oldCardImage = CardImages.findOneByName(card._id);
		if (oldCardImage) {
			// console.log('cardImage upload oldCardImage ->', file);
			CardImages.deleteFile(oldCardImage._id);
		}
		CardImages.updateFileNameById(file._id, card._id);
		// console.log('cardImage upload finished ->', file);
	},
	groupImagesOnFinishUpload(file) {
		if (Meteor.userId() !== file.userId && !hasPermission(Meteor.userId(), 'edit-other-user-info')) {
			throw new Meteor.Error('error-not-allowed', 'Change group image is not allowed');
		}
		// update file record to match user's username
		const user = Users.findOneById(file.userId);
		const oldCardImage = GroupImages.findOneByName(file.rid);
		if (oldCardImage) {
			GroupImages.deleteFile(oldCardImage._id);
		}
		GroupImages.updateFileNameById(file._id, file.rid);
		// console.log('cardImage finished ->', file);
	},
...
export class FileUploadClass {
	...
	deleteByCardId(cardId) {
		const file = this.model.findOneByCardId(cardId);
