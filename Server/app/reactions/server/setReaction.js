...
import { Messages, EmojiCustom, Rooms, Cards } from '../../models';
...
async function setReaction(room, user, card, message, reaction, shouldReact) {
	...
	if (Array.isArray(room.muted) && room.muted.indexOf(card._id) !== -1) {
		throw new Meteor.Error('error-not-allowed', TAPi18n.__('You_have_been_muted', {}, user.language), {
			rid: room._id,
		});
	}

	const userAlreadyReacted = Boolean(message.reactions) && Boolean(message.reactions[reaction]) && message.reactions[reaction].usernames.indexOf(card.username) !== -1;
	...
	if (userAlreadyReacted) {
		removeUserReaction(message, reaction, card.username);
		...
		message.reactions[reaction].usernames.push(card.username);
		Messages.setReactions(message._id, message.reactions);
		if (isTheLastMessage(room, message)) {
			Rooms.setReactionsInLastMessage(room._id, message);
		}
...
export const executeSetReaction = async function(reaction, messageId, cardId, shouldReact) {
	...
	return setReaction(room, user, card, message, reaction, shouldReact);
};

Meteor.methods({
	setReaction(reaction, messageId, cardId, shouldReact) {
		try {
			return Promise.await(executeSetReaction(reaction, messageId, cardId, shouldReact));
		} catch (e) {
