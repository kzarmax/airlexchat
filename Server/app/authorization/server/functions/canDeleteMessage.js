...
export const canDeleteMessageAsync = async (uid, cardId, { c, rid, ts }) => {
	...
	const allowed = allowedToDeleteAny || (cardId === c._id && await hasPermissionAsync(uid, 'delete-own-message'));
	if (!allowed) {
		return false;
	}
...
export const canDeleteMessage = (uid, cardId, { c, rid, ts }) => Promise.await(canDeleteMessageAsync(uid, cardId,{ c, rid, ts }));
