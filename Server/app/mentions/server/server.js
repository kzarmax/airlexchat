...
import { Users, Subscriptions, Rooms, Cards } from '../../models';
import { api } from '../../../server/sdk/api';

const mention = new MentionsServer({
	pattern: () => settings.get('UTF8_Names_Validation'),
	messageMaxAll: () => settings.get('Message_MaxAll'),
	getUsers: (usernames) => Cards.find({ username: { $in: _.unique(usernames) } }, { fields: { _id: true, username: true } }).fetch(),
	getChannelMembers: (rid) => { const subscriptions = Subscriptions.find({ rid }, { fields: {'c': 1} }).fetch(); return subscriptions.map(sub => sub.c); },
	getUser: (cardId) => Cards.findOneById(cardId),
