...
export class SubscriptionsRaw extends BaseRaw<T> {
	...
	findOneByRoomIdAndCardId(rid: string, cardId: string, options: FindOneOptions<T> = {}): Promise<T | undefined> {
		const query = {
			rid: rid,
			'c._id': cardId,
		};
	...
	async isUserInRole(uid: string, cardId: string, roleName: string, rid: string): Promise<T | undefined> {
		if (rid == null) {
			return;
		}
	...
	setAsReadByRoomIdAndCardId(rid: string, cardId: string, alert = false, options: FindOneOptions<T> = {}): ReturnType<BaseRaw<T>['update']> {
		const query: FilterQuery<T> = {
			rid,
			'c._id': cardId,
		};
